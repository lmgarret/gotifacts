...
- Diagrams are ```` ```mermaid ```` blocks rendered to SVG at build time by a
  headless Chromium (via Playwright). Install it once with `cd docs && npx
  playwright install chromium` so `npm run build` can render locally; CI
  installs it the same way before building. See `docs/README.md`.
